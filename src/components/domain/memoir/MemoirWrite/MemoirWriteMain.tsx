import React, { useState, useEffect } from "react";

import MeetingCard from "../MemoirCommon/MeetingCard";
import ProjectContainer from "./ProjectContainer";
import styles from "./MemoirWriteMain.module.scss";

const Memoir_write_main = ({
  projectsAll,
  refetchProjects,
  contribution,
  setContribution,
  role,
  setRole,
  feeling,
  setFeeling,
  done,
  setDone,
  todo,
  setTodo,
  projectId,
  setProjectId,
  chosenProjectBgColor,
  chosenProjectTextColor,
  setChosenProjectBgColor,
  setChosenProjectTextColor,
  meeting, //meetingid받기위해서...
}) => {
  const [chosenProject, setChosenProject] = useState<string>("");

  useEffect(() => {
    if (meeting?.projectId) {
      setProjectId(meeting.projectId); // 페이지 진입/meeting 로드 시 1회 반영
    }
  }, [meeting?.projectId]);

  useEffect(() => {
    setChosenProject(meeting?.projectName ?? "");
  }, [meeting?.projectName]);

  return (
    <div className={styles.main_ctn}>
      <div className={styles.meeting_card_ctn}>
        <p className={styles.write_title}>작성할 회의</p>
        <MeetingCard
          chosenProject={chosenProject}
          chosenProjectBgColor={chosenProjectBgColor}
          chosenProjectTextColor={chosenProjectTextColor}
          meeting={meeting}
        />
      </div>
      <div className={styles.contribution_role}>
        <div className={styles.ctn_in_common}>
          <p className={styles.write_title}>
            기여도<span className={styles.star}>*</span>
          </p>
          <input
            type="text"
            className={`${styles["in-common"]} ${styles.contribution_input}`}
            value={contribution}
            placeholder="0"
            onChange={(e) => setContribution(e.target.value)}
          />
        </div>
        <div className={styles.ctn_in_common}>
          <p className={styles.write_title}>
            맡은 역할<span className={styles.star}>*</span>
          </p>
          <input
            type="text"
            className={styles["in-common"]}
            value={role}
            placeholder="맡은 역할"
            onChange={(e) => setRole(e.target.value)}
          />
        </div>
      </div>
      <div className={styles.ctn_in_common}>
        <p className={styles.write_title}>
          느낀 점<span className={styles.star}>*</span>
        </p>
        <textarea
          className={`${styles["in-common"]} ${styles.feel_ctn}`}
          value={feeling}
          placeholder="미팅에서 만족한 부분, 문제점 그리고 시도해볼 것이 있나요?"
          onChange={(e) => setFeeling(e.target.value)}
        ></textarea>
      </div>
      <div className={styles.ctn_in_common}>
        <p className={styles.write_title}>한 일</p>
        <input
          type="text"
          className={styles["in-common"]}
          value={done}
          placeholder="이번 시간에 한 일을 작성해 주세요"
          onChange={(e) => setDone(e.target.value)}
        />
      </div>
      <div className={styles.ctn_in_common}>
        <p className={styles.write_title}>할 일</p>
        <input
          type="text"
          className={styles["in-common"]}
          value={todo}
          placeholder="다음 시간까지 할 일을 작성해주세요"
          onChange={(e) => setTodo(e.target.value)}
        />
      </div>
      <ProjectContainer
        projectsAll={projectsAll}
        refetchProjects={refetchProjects}
        projectId={projectId}
        setProjectId={setProjectId}
        chosenProjectBgColor={chosenProjectBgColor}
        chosenProjectTextColor={chosenProjectTextColor}
        setChosenProjectTextColor={setChosenProjectTextColor}
        setChosenProjectBgColor={setChosenProjectBgColor}
        chosenProject={chosenProject}
        setChosenProject={setChosenProject}
      />
    </div>
  );
};

export default Memoir_write_main;
