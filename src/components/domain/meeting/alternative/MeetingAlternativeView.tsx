...
const MeetingAlternativeView = ({ alternativeTimes, meetingId }: Props) => {
  ...
  const [clickedEventNum, setClickedEventNum] = useState<ClickedEventProps | null>(null);
  ...
  const handleButtonClick = (e: EventApi) => {
    setClickedEventNum(e.extendedProps as ClickedEventProps);
    setPopupInfo(null);
  };
      ...
      {!loading && firstDate && duration > 0 && data && (
        <div className={styles.meetingAlternativeView__body}>
          <FullCalendar
            ...
            events={data}
            eventClassNames={(arg) => {
              return arg.event.extendedProps.index === clickedEventNum?.index
                ? [styles.selectedEvent] // 선택된 대인시간만 클래스를 부여
                : [];
            }}
            eventContent={(arg) =>
              arg.event.extendedProps.index === clickedEventNum?.index ? (
